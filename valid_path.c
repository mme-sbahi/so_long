...
/*   Updated: 2023/02/23 12:51:01 by mmesbahi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_flood_fill(t_ptr *data)
{
	...
	flood_fill_c(tmp, data->pos_player.x, data->pos_player.y);
	while(tmp[i])
	{
	...
	ft_free(tmp,ft_strlen_2d(tmp));
	if(count != 0)
		ft_error("error backtracking\n", data->tab);
	count = 0;
	i = 0;
	tmp = dup_2d_arr(data->tab);
	flood_fill_e(tmp, data->pos_player.x, data->pos_player.y);
	while(tmp[i])
	{
